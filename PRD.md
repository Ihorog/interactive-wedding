...
- **Purpose**: Bring wedding moments to life with motion and sound, enable users to easily add ceremony and celebration videos that become the emotional centerpiece of each section
- **Trigger**: Upload via AI chat file attachment icon → AI processes and generates thumbnails → Videos assigned to sections via Media Manager → Videos displayed in featured layout → Click to play inline or in lightbox
- **Progression**: Click paperclip icon in AI chat → Select video files (multi-select supported) → Upload progress bar shows percentage → AI automatically extracts metadata (duration, dimensions, size) and generates thumbnail → Videos stored in KV with dataURL → User opens Media Manager (folder icon) → Filter by section → Select video → Edit title, description, section assignment, add tags → Save changes → Video appears in section's VideoShowcase (featured layout) → Click video thumbnail → VideoPlayer opens with custom controls → Play/pause with center button, hover shows controls, seek bar at bottom, mute button, time display, smooth animations
- **Success criteria**: Videos upload smoothly with real-time progress indicator (0-100%), thumbnails auto-generate from video frame, playback smooth on mobile/desktop (responsive video player), file size limit enforced (100MB max, clear error message), videos organized in Media Manager with filter/search, videos persist across sessions via KV storage, VideoShowcase component displays videos prominently with featured/grid layout, custom video controls match wedding theme (golden highlights, volumetric cards), videos work in both gallery view and full lightbox dialog, metadata displays correctly (duration formatted as MM:SS, file size in KB/MB)
